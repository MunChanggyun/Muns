...
import {cCodeList, searchComp, latestSearch} from '../../modules/company'
...
const Company: React.FC<RouteComponentProps> = ({ history }:RouteComponentProps) => {
    ...
    const {codeListError, company, latestCompany} = useSelector(({company}: RootState) => ({
    ...
    useEffect(() => {
        if (!latestCompany) {
            dispath(latestSearch())
        }

        console.log(latestCompany);
    },[dispath, latestCompany])
